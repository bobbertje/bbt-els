    ...
    while true do
        ...
        if isVehicleELS and canControlELS and networkSessionActive then
            ...
            if(GetLastInputMethod(0)) then
                ...
                DisableControlAction(0, keyboard.pitje, true)
                ...
                if IsDisabledControlPressed(0, keyboard.modifyKey) then
                    ...
                    if IsDisabledControlJustReleased(0, keyboard.guiKey) then
                        ...
                        PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 5)
                        end
                    ...
                    if IsDisabledControlJustReleased(0, keyboard.pitje) then
                        if playButtonPressSounds then
                            PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 5)
                        end
                        TriggerServerEvent("els:setpitjeState_s")
                    end
                    if IsDisabledControlJustReleased(0, keyboard.takedown) then
                        if playButtonPressSounds then
                        PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 5)
                ...
                if GetVehicleClass(GetVehiclePedIsUsing(PlayerPedId())) == 18 then
                    if (elsVehs[GetVehiclePedIsUsing(PlayerPedId())] ~= nil) then
                        if elsVehs[GetVehiclePedIsUsing(PlayerPedId())].stage == 3 then
                            if IsDisabledControlJustReleased(0, keyboard.siren.tone_one) or IsDisabledControlJustReleased(0, keyboard.stageAlt) then
                            ...
                            if IsDisabledControlJustReleased(0, keyboard.siren.tone_seven) then
                                setSirenStateButton(7)
                            end
                            if IsDisabledControlJustReleased(0, keyboard.siren.tone_eight) then
                                setSirenStateButton(8)
                            end
                            if IsDisabledControlJustReleased(0, keyboard.siren.tone_nine) then
                                setSirenStateButton(9)
                            end
                            if IsDisabledControlJustReleased(0, keyboard.siren.tone_ten) then
                                setSirenStateButton(10)
                            end
                        end
                ...
                if canControlELS then
                    ...
                    if vehN ~= 0 then
                        ...
                        if (panelType == "original") then
                            ...
                            Draw("MODIFORCE SYSTEMS", 0, 0, 0, 255, 0.92 + panelOffsetX, 0.825 + panelOffsetY, 0.25, 0.25, 1, true, 0)
...
-- 								--debugPrint("Index voor huidig voertuig (".. vehN .. ") was nil (ongeldig), thread restarten...", true, true)
--                                break 
-- 							   --return
--                             end
...
--                                     end
--                                     if i == 12 then
--                                         DrawLightWithRange(coords.x + els_Vehicles[vehN].extras[12].env_pos.x, coords.y + els_Vehicles[vehN].extras[12].env_pos.y, coords.z + els_Vehicles[vehN].extras[12].env_pos.z, 255, 255, 255, 200.0, environmentLightBrightness)
--                                     end
--                                 else
--                                     if i == 11 then
...
--                                     end
--                                     if i == 12 then
--                                         DrawLightWithRange(coords.x, coords.y, coords.z, 255, 255, 255, 200.0, environmentLightBrightness)
--                                     end
--                                 end
--                             end
--                         end
        ...
        for k,v in pairs(elsVehs) do
            if v ~= nil and DoesEntityExist(k) and #(GetEntityCoords(k)-GetEntityCoords(PlayerPedId())) <= vehicleSyncDistance then
                ...
                if getVehicleVCFInfo(k) == false then
                    --debugPrint("probleem gevonden in VCF informatie voor voertuig " .. k .. ", maak een screenshot voor developers", true, true)
                    --TriggerServerEvent("els:restart")
