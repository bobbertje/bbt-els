...
RegisterNetEvent("els:setpitjeState_c")
AddEventHandler("els:setpitjeState_c", function(sender)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if player_s ~= -1 then 
        if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
            if IsPedInAnyVehicle(ped_s, false) then
                ...
                if IsVehicleExtraTurnedOn(veh, 10) then
                    setExtraState(veh, 10, 1)
                    -- Zet ook blauw blauw uit ala dat aan staat
                    -- Als de sirene aan is, deze uitzetten als je blauwblauw uitzet
                    if m_siren_state[veh] == 1 or m_siren_state[veh] == 2 then
    ...
    if player_s ~= -1 then 
        if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
            if IsPedInAnyVehicle(ped_s, false) then
                ...
                if IsVehicleExtraTurnedOn(veh, 10) or IsVehicleExtraTurnedOn(veh, 11) then
                    setExtraState(veh, 10, 1)
...
function downOneStage()
    ...
    if(elsVehs[vehNetID] ~= nil and elsVehs[vehNetID].stage ~= nil) then
        newStage = elsVehs[vehNetID].stage - 3
    end
